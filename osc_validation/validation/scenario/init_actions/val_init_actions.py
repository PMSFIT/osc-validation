...
def _base_actor(speed_mps: float | None = None) -> InitActionOracleActor:
...
@pytest.mark.trajectory
def test_missing_init_speed_action_keeps_actor_stationary(
    odr_file: Path,
    generate_tool_trace: Callable,
    tmp_path: Path,
):
    ...
    case_result = build_init_teleport_action_case(
        InitActionCaseSpec(
            output_xosc_path=tmp_path / "missing_init_speed_action.xosc",
            output_reference_channel_spec=ChannelSpecification(
                path=tmp_path / "reference_missing_init_speed_action.mcap",
                message_type="SensorView",
                metadata={
                    "net.asam.osi.trace.channel.description": "Reference trace for missing Init SpeedAction validation"
                },
            ),
            actors=[_base_actor()],
            duration_s=0.5,
    ...
    tool_trace_channel_spec = generate_tool_trace(
        ...
        odr_path=odr_file,
        osi_output_spec=ChannelSpecification(
            path=tmp_path / "tool_trace_missing_init_speed_action.mcap",
            message_type="SensorView",
            metadata={
                "net.asam.osi.trace.channel.description": "Tool trace for missing Init SpeedAction validation"
            },
        ),
        log_path=tmp_path,
        rate=rate,
    ...
    assert metric_result.max_time_error <= 0.01
    assert metric_result.max_xy_error < 0.1
    assert metric_result.max_planar_speed_error < 0.1
